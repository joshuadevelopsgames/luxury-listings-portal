...
/**
 * Slack-style chat widget for team communication
 * Connects to real Slack workspace when token is configured
 */
const SlackChatWidget = () => {
  ...
  // Check if Slack is configured on mount
  useEffect(() => {
    setIsConfigured(slackService.isConfigured());
  }, []);

  // Load channels when widget opens
  useEffect(() => {
  ...
  const loadChannels = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const channelList = await slackService.getChannels();
  ...
  const loadMessages = async (channelId, silent = false) => {
    try {
      ...
      const msgs = await slackService.getMessages(channelId, 50);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await slackService.sendMessage(selectedChannel.id, messageText);
  ...
  // Demo/unconfigured mode UI
  const renderDemoMode = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
      <div className="w-16 h-16 bg-purple-100 rounded-xl flex items-center justify-center mb-4">
        <MessageSquare className="w-8 h-8 text-purple-600" />
      </div>
      <h3 className="font-semibold text-gray-900 mb-2">Connect to Slack</h3>
      <p className="text-sm text-gray-600 mb-4">
        Add your Slack token to enable team chat.
      </p>
      <div className="bg-gray-50 rounded-lg p-3 text-left w-full">
        <p className="text-xs text-gray-500 mb-1">Add to .env.local:</p>
        <code className="text-xs text-purple-700 break-all">
          REACT_APP_SLACK_TOKEN=xoxp-your-token
        </code>
      </div>
    </div>
  );
  ...
  // CORS error UI
  const renderCorsError = () => (
    <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
      <div className="w-16 h-16 bg-orange-100 rounded-xl flex items-center justify-center mb-4">
        <AlertCircle className="w-8 h-8 text-orange-600" />
      </div>
      <h3 className="font-semibold text-gray-900 mb-2">CORS Blocked</h3>
      <p className="text-sm text-gray-600 mb-4">
        Slack's API doesn't allow browser requests directly. You'll need to set up a proxy.
      </p>
      <div className="bg-gray-50 rounded-lg p-3 text-left w-full text-xs">
        <p className="text-gray-700 mb-2">Options:</p>
        <ul className="text-gray-600 space-y-1">
          <li>• Deploy a Firebase Function proxy</li>
          <li>• Use a Vercel serverless function</li>
  ...
  return (
    <div className="fixed bottom-4 right-20 z-50">
      ...
      {!isOpen && (
        <button
          onClick={toggleChat}
          className="bg-[#4A154B] hover:bg-[#611f69] text-white rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105"
        ...
        <div className="bg-white rounded-lg shadow-2xl border border-gray-200 w-96 h-[500px] flex flex-col overflow-hidden">
          ...
          <div className="bg-[#4A154B] text-white px-4 py-3 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <span className="font-semibold">Slack Chat</span>
                {selectedChannel && (
            ...
            <div className="flex items-center space-x-1">
              <button
                onClick={() => selectedChannel && loadMessages(selectedChannel.id)}
          ...
          {/* Channel Selector */}
          {isConfigured && !error && channels.length > 0 && (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline space-x-2">
                        ...
                        <span className="text-xs text-gray-500">
                          {slackService.formatTimestamp(message.ts)}
                        </span>
                      </div>
          ...
          {/* Input Area */}
          {isConfigured && !error && selectedChannel && (
