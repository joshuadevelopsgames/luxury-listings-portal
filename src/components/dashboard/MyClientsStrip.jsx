...
const REPORT_STATUS = {
  complete: { label: 'Report done', dot: 'bg-positive', text: 'text-[#1a7a2e] dark:text-positive' },
  partial: { label: 'Report partial', dot: 'bg-warning', text: 'text-[#c26a00] dark:text-warning' },
  missing: { label: 'Report missing', dot: 'bg-danger', text: 'text-danger' },
};
...
const packageHealth = (client) => {
  ...
  if (percentage <= 20) return 'var(--ds-danger)';
  if (percentage <= 50) return 'var(--ds-warning)';
  return 'var(--ds-positive)';
};
...
const ClientCard = ({ client, status, showReportStatus }) => {
  ...
  return (
    <div className="p-3 sm:p-4 rounded-xl bg-surface border border-gray-200 dark:border-white/5 flex flex-col gap-2.5 isolate">
      <div className="flex items-start gap-2 min-w-0">
        {client.logo ? (
          <img src={client.logo} alt="" className="w-7 h-7 rounded-full object-cover shrink-0 border border-hairline" />
        ) : (
          <div className="w-7 h-7 rounded-full shrink-0 bg-ink dark:bg-white/20 flex items-center justify-center text-white text-[10px] font-semibold">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-ink-muted truncate">{client.packageType || client.platform || 'Standard'}</p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[11px] text-ink-muted">Posts</span>
          <span className="text-[11px] font-medium text-ink">{postsUsed}/{packageSize}</span>
        </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <h2 className="text-[15px] sm:text-[17px] font-semibold text-ink flex items-center gap-2">
          <Users className="w-4 h-4 text-brand" strokeWidth={1.75} />
          My Clients
          {clients.length > 0 && <span className="text-ink-muted font-normal">{clients.length}</span>}
        </h2>
        <Link to={clientsPath} className="text-[13px] text-brand font-medium hover:underline flex items-center gap-1">
          View all
      ...
      {clients.length === 0 ? (
        <div className="rounded-xl bg-surface border border-gray-200 dark:border-white/5 p-8 text-center">
          <Users className="w-10 h-10 text-ink-muted mx-auto mb-2" strokeWidth={1.5} />
          <p className="text-[15px] font-medium text-ink">No clients assigned to you yet</p>
          <p className="text-[13px] text-ink-muted mt-1">They'll appear here once a manager assigns them.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
